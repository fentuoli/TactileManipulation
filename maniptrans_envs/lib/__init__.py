...
# Don't auto-import old IsaacGym envs - they need migration
# Import specific modules directly when needed

__ALL__ = ["make"]

# TASK_MAP is defined lazily when needed for training
_TASK_MAP = None

def get_task_map():
    """Get task map, importing only when needed."""
    global _TASK_MAP
    if _TASK_MAP is None:
        # This will fail if IsaacGym is not available
        # For IsaacLab environments, use the new maniptrans_envs.tasks module
        from .envs.tasks import (
            DexHandImitatorRHEnv, DexHandImitatorLHEnv,
            DexHandManipRHEnv, DexHandManipLHEnv, DexHandManipBiHEnv
        )
        _TASK_MAP = {
