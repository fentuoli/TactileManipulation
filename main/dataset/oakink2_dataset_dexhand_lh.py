...
class OakInk2DatasetDexHandLH(ManipData):
    ...
    def __getitem__(self, index):
        ...
        assert (
            left_hand_range is not None
        ), f"Left hand data is empty. Please check if {index} is a right-hand-only task."

        if right_hand_range is not None:
            left_hand_range = intersection(left_hand_range, right_hand_range)

        program_info_selected = program_info[list(program_info.keys())[stage]]

        frame_id_list = [f for f in frame_id_list if left_hand_range[0] <= f <= left_hand_range[1]]
        ...
        mano_joints = {
            ...
            "index_tip": smplx_results.vertices[:, 4970].detach(),  # reselect tip
            ...
            "middle_tip": smplx_results.vertices[:, 5082].detach(),  # reselect tip
            ...
            "pinky_tip": smplx_results.vertices[:, 5310].detach(),  # reselect tip
            ...
            "ring_tip": smplx_results.vertices[:, 5193].detach(),  # reselect tip
            ...
            "thumb_tip": smplx_results.vertices[:, 5362].detach(),  # reselect tip
        }
        ...
        # Use trimesh object directly for point sampling
        rs_verts_obj = self.random_sampling_pc(obj_mesh_trimesh)
...
if __name__ == "__main__":
    fdata = OakInk2DatasetDexHandLH(data_dir="data/OakInk-v2", mujoco2gym_transf=torch.eye(4, device="cuda:0"))
    print(fdata[34])
