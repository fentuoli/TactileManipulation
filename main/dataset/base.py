...
def sample_points_from_mesh(mesh, num_samples=1000):
...
class ManipData(Dataset, ABC):
    ...
    def random_sampling_pc(self, mesh):
        ...
        Returns:
            torch.Tensor of shape (1000, 3) on self.device
        """
        ...
        rs_verts_obj = sample_points_from_mesh(mesh, 1000).to(self.device)

        # reset seed
        np.random.set_state(numpy_random_state)
        torch.random.set_rng_state(torch_random_state)
        if torch_random_state_cuda is not None:
