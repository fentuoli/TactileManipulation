...
# ============================================================================
# Replacement functions for pytorch3d.transforms
# Using scipy and custom torch implementations
# ============================================================================
...
def axis_angle_to_matrix(axis_angle: torch.Tensor) -> torch.Tensor:
    ...
    angle = torch.norm(axis_angle_flat, dim=-1, keepdim=True)  # (N, 1)

    # Normalize to get axis (avoid division by zero)
    axis = axis_angle_flat / (angle + 1e-8)  # (N, 3)
    ...
    eye = torch.eye(3, device=device, dtype=dtype).unsqueeze(0).expand(axis_angle_flat.shape[0], -1, -1)

    rotation_matrix = eye + sin_angle.unsqueeze(-1) * K + (1 - cos_angle.unsqueeze(-1)) * K2

    # Handle zero angle case (identity matrix)
    small_angle_mask = (angle < 1e-6).squeeze(-1)
    if small_angle_mask.any():
        rotation_matrix[small_angle_mask] = eye[small_angle_mask]
...
def axis_angle_to_quaternion(axis_angle: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Tensor of shape (..., 4) in (w, x, y, z) format
    """
    batch_shape = axis_angle.shape[:-1]
    device = axis_angle.device
    dtype = axis_angle.dtype
    ...
    quaternion = torch.cat([w, xyz], dim=-1)  # (N, 4)

    # Handle zero angle case
    small_angle_mask = (angle < 1e-8).squeeze(-1)
    if small_angle_mask.any():
        quaternion[small_angle_mask] = torch.tensor([1.0, 0.0, 0.0, 0.0], device=device, dtype=dtype)
...
def quaternion_to_axis_angle(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    quaternion_flat = quaternion.reshape(-1, 4)

    # Normalize quaternion
    quaternion_flat = quaternion_flat / (torch.norm(quaternion_flat, dim=-1, keepdim=True) + 1e-8)

    # Ensure w is positive (q and -q represent the same rotation)
    quaternion_flat = torch.where(
        quaternion_flat[:, 0:1] < 0,
        -quaternion_flat,
        quaternion_flat
    )

    w = quaternion_flat[:, 0:1]
    xyz = quaternion_flat[:, 1:4]
    ...
    small_angle_mask = (sin_half_angle < 1e-8).squeeze(-1)
    if small_angle_mask.any():
        axis_angle[small_angle_mask] = torch.zeros(3, device=device, dtype=dtype)
...
def matrix_to_quaternion(matrix: torch.Tensor) -> torch.Tensor:
    ...
    mask1 = trace > 0
    if mask1.any():
        s = torch.sqrt(trace[mask1] + 1) * 2  # s = 4 * w
        quaternion[mask1, 0] = 0.25 * s
        quaternion[mask1, 1] = (matrix_flat[mask1, 2, 1] - matrix_flat[mask1, 1, 2]) / s
        quaternion[mask1, 2] = (matrix_flat[mask1, 0, 2] - matrix_flat[mask1, 2, 0]) / s
        quaternion[mask1, 3] = (matrix_flat[mask1, 1, 0] - matrix_flat[mask1, 0, 1]) / s

    # Case 2: m00 is largest diagonal
    mask2 = ~mask1 & (matrix_flat[:, 0, 0] > matrix_flat[:, 1, 1]) & (matrix_flat[:, 0, 0] > matrix_flat[:, 2, 2])
    if mask2.any():
        s = torch.sqrt(1 + matrix_flat[mask2, 0, 0] - matrix_flat[mask2, 1, 1] - matrix_flat[mask2, 2, 2]) * 2
        quaternion[mask2, 0] = (matrix_flat[mask2, 2, 1] - matrix_flat[mask2, 1, 2]) / s
        quaternion[mask2, 1] = 0.25 * s
        quaternion[mask2, 2] = (matrix_flat[mask2, 0, 1] + matrix_flat[mask2, 1, 0]) / s
        quaternion[mask2, 3] = (matrix_flat[mask2, 0, 2] + matrix_flat[mask2, 2, 0]) / s

    # Case 3: m11 is largest diagonal
    mask3 = ~mask1 & ~mask2 & (matrix_flat[:, 1, 1] > matrix_flat[:, 2, 2])
    if mask3.any():
        s = torch.sqrt(1 + matrix_flat[mask3, 1, 1] - matrix_flat[mask3, 0, 0] - matrix_flat[mask3, 2, 2]) * 2
        quaternion[mask3, 0] = (matrix_flat[mask3, 0, 2] - matrix_flat[mask3, 2, 0]) / s
        quaternion[mask3, 1] = (matrix_flat[mask3, 0, 1] + matrix_flat[mask3, 1, 0]) / s
        quaternion[mask3, 2] = 0.25 * s
        quaternion[mask3, 3] = (matrix_flat[mask3, 1, 2] + matrix_flat[mask3, 2, 1]) / s

    # Case 4: m22 is largest diagonal
    mask4 = ~mask1 & ~mask2 & ~mask3
    if mask4.any():
        s = torch.sqrt(1 + matrix_flat[mask4, 2, 2] - matrix_flat[mask4, 0, 0] - matrix_flat[mask4, 1, 1]) * 2
        quaternion[mask4, 0] = (matrix_flat[mask4, 1, 0] - matrix_flat[mask4, 0, 1]) / s
        quaternion[mask4, 1] = (matrix_flat[mask4, 0, 2] + matrix_flat[mask4, 2, 0]) / s
        quaternion[mask4, 2] = (matrix_flat[mask4, 1, 2] + matrix_flat[mask4, 2, 1]) / s
        quaternion[mask4, 3] = 0.25 * s
...
def quaternion_to_matrix(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    w, x, y, z = quaternion_flat[:, 0], quaternion_flat[:, 1], quaternion_flat[:, 2], quaternion_flat[:, 3]
...
def matrix_to_rotation_6d(matrix: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Tensor of shape (..., 6) - first two columns of the rotation matrix
    """
...
def euler_angles_to_matrix(euler_angles: torch.Tensor, convention: str = "XYZ") -> torch.Tensor:
    ...
    Args:
        euler_angles: Tensor of shape (..., 3)
        convention: Rotation convention (e.g., "XYZ", "ZYX")
    ...
    euler_flat = euler_angles.reshape(-1, 3).cpu().numpy()

    # Use scipy for conversion
    r = R.from_euler(convention.lower(), euler_flat)
    matrix = torch.from_numpy(r.as_matrix().astype(np.float32)).to(device=device, dtype=dtype)
...
def matrix_to_euler_angles(matrix: torch.Tensor, convention: str = "XYZ") -> torch.Tensor:
    ...
    matrix_flat = matrix.reshape(-1, 3, 3).cpu().numpy()

    # Use scipy for conversion
    r = R.from_matrix(matrix_flat)
    euler = torch.from_numpy(r.as_euler(convention.lower()).astype(np.float32)).to(device=device, dtype=dtype)

    return euler.reshape(*batch_shape, 3)


# ============================================================================
# End of pytorch3d replacement functions
# ============================================================================
